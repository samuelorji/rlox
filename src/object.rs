...
impl ObjString {
    ...
    pub fn as_str(&self) -> &str {
        unsafe {
            std::str::from_utf8(std::slice::from_raw_parts(self.ptr, self.length))
    ...
    pub fn from_buffer(buffer: &[u8]) -> Self {
        ...
        unsafe {
            let layout =
                Layout::array::<u8>(len_of_string).expect("cannot create layout for string");
...
impl Debug for Obj {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Obj::FUNCTION(function @ ObjFunction { .. }) => f.write_str(function.name.as_str()),
            ...
            Obj::CLASS(class @ ObjClass { .. }) => f.write_str(class.name.as_str()),
            Obj::INSTANCE(instance @ ObjInstance { .. }) => f.write_str(&format!("{:?}", instance)),
...
impl Obj {
    pub fn free(self) {
        match self {
            Obj::STRING(string @ ObjString { .. }) => string.free(),
            Obj::FUNCTION(function @ ObjFunction { .. }) => {
                function.name.free();
            }
            Obj::NATIVE_FUNCTION(native @ NativeFunction { .. }) => native.name.free(),
            Obj::CLOSURE(closure @ ObjClosure { .. }) => {}
            Obj::CLASS(class @ ObjClass { .. }) => class.name.free(),
            Obj::INSTANCE(instance @ ObjInstance { .. }) => instance.class.name.free(),

            Obj::BOUND_METHOD(method @ ObjBoundMethod { .. }) => method.method.function.name.free(),
        }
    }
}
